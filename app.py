...
if "chunks" in st.session_state and "embeddings" in st.session_state:
    ...
    if query:
        ...
        st.subheader("🧠 Friendly Answer (Summarized)")
        summarizer = pipeline("summarization", model="sshleifer/distilbart-cnn-12-6")
        summary = summarizer(combined_text, max_length=120, min_length=40, do_sample=False)[0]["summary_text"]

        # Optional enhancement with explanation context
        st.markdown(f"""
        <div style='background-color: #e8f5e9; padding: 1rem; border-left: 6px solid #4caf50; border-radius: 8px;'>
        <strong>💬 Summary:</strong> {summary}
        <br><small>Based on key segments from the paper for a concise, easy-to-understand overview.</small>
        </div>
        """, unsafe_allow_html=True)
